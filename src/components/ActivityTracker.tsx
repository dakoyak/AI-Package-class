...
const PATH_LABELS: Array<{
  ...
  entry: (pathname: string) => { category: string; label: string; detail: string } | null;
}> = [
  {
    test: (path) => path === '/ai-literacy',
    entry: () => ({ category: 'AI 리터러시', label: '인트로', detail: '미션 선택 화면을 살펴봤어요.' }),
  },
  {
    ...
    entry: (path) => {
      ...
      const missionName = mission === 'guardrail' ? '안전장치 확인' : '편견 찾기';
      return {
        category: 'AI 리터러시',
  ...
  },
  {
    ...
    entry: () => ({ category: '창의력', label: 'AI 상상 스파링', detail: '엉뚱한 반론으로 이야기를 새로 썼어요.' }),
  },
  {
    test: (path) => path === '/creative-classroom/creativity/art',
    entry: () => ({ category: '창의력', label: 'AI 아트 워크숍', detail: '작품에 새로운 화풍을 입혔어요.' }),
  },
  {
    test: (path) => path === '/creative-classroom/creativity/writing',
    entry: () => ({ category: '창의력', label: 'AI 글쓰기 듀오', detail: '맞춤 글쓰기 안내를 받았어요.' }),
  },
  {
    test: (path) => path === '/immersive/history',
    entry: () => ({ category: '몰입형 체험', label: '역사 인터뷰', detail: 'AI 세종대왕과 대화를 나눴어요.' }),
  },
  {
    test: (path) => path === '/immersive/coach',
    entry: () => ({ category: '몰입형 체험', label: 'AI 체육 코치', detail: '움직임을 따라 하며 연습했어요.' }),
  },
  {
    test: (path) => path === '/collaboration/smart-discussion',
    entry: () => ({ category: '논리/협업', label: '곰곰이 스마트 토론', detail: '말하기와 경청 연습을 해봤어요.' }),
  },
];
